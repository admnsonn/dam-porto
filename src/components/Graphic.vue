<template>
	<section id="frontend"
		...
		<section class="mt-4 md:flex-nowrap mx-auto md:flex gap-5 flex-wrap">
			<template v-for="(gra, x) in currentPage" :key="x">
				<Cgrafik :content="gra" />
			</template>
		</section>
...
import Cgrafik from '@/components/Cgrafik.vue'
import grafik from '@/contents/grafik.js'
import { ref } from 'vue'

const divideCard = 4
const pages = []
let loop = Math.floor(grafik.length / divideCard)

for (let x = 0; x <= loop; x++) {
	if (grafik.length > 0) pages.push(grafik.splice(0, divideCard))
}
