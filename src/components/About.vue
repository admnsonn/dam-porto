<template>
	<section id="about"
		class="py-8 bg-gradient-to-b from-gray-50 to-indigo-50 px-8 md:px-20 dark:from-[#18181b] dark:to-[#18181b] dark:text-[#f8fafc]">
				...
				<div>
					<template v-for="(edu, x) in educations" :key="x">
						<div data-aos="zoom-in"
							class="mb-3 border-l-4 border-indigo-600 dark:border-[#38bdf8] rounded py-2 pl-3">
							<span class="flex items-center gap-3">
								<i class="fas fa-building text-indigo-600 dark:text-[#38bdf8]"></i>
								<p class="font-bold text-gray-700 dark:text-gray-300">{{ edu.job }}</p>
							</span>
							<p class="font-medium text-gray-700 dark:text-gray-300">{{ edu.institusi }}</p>
			...
			<div class="md:w-5/12">
				...
				<ul>
					<template v-for="(skil, x) in skils" :key="x">
						<li data-aos="zoom-in"
							class="mb-3 border-l-4 border-indigo-600 dark:border-[#38bdf8] rounded py-2 pl-3 font-medium text-gray-700 dark:text-gray-300">
							{{ skil }}</li>
					</template>
				</ul>
			</div>
				...
				<div>
					<template v-for="(exper, x) in expers" :key="x">
						<div data-aos="zoom-in"
							...
							<span class="flex items-center gap-3">
								<i class="fas fa-building text-indigo-600 dark:text-[#38bdf8]"></i>
								<p class="font-bold text-gray-700 dark:text-gray-300">{{ exper.job }}</p>
							</span>
							<p class="font-medium text-gray-700 dark:text-gray-300">{{ exper.institusi }}</p>
